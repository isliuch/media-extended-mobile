...
- Android 可选安装独立的 Media Extended 截图助手 APK。“辅助 APK 一键截图”按钮使用系统 MediaProjection 授权抓取当前屏幕，自动裁剪播放器、识别当前视频时间并插入此前聚焦的笔记。1.0.3 起可选启用无障碍服务：首张截图没有可读时间时，助手会自动点击视频使控制条显示，再用第二张截图识别时间。1.0.4 起新增独立的“高清源视频帧”按钮：YouTube/B站由内置 yt-dlp 解析 H.264/MP4 视频源，再由 Android 解码指定时间的原尺寸 PNG；失败或超时会自动回退到原屏幕截图。原相机按钮和图片导入按钮均保留。
...
- “高清源视频帧”首次使用需要初始化约 80 MB 的内置运行时，解析可能较慢；YouTube 的部分格式可能受 PO Token 限制，B站登录/大会员清晰度不会自动继承 Obsidian WebView 的 Cookie。该模式默认优先兼容 Android 8.1 的 H.264/MP4 视频流，并在 75 秒后安全回退。
