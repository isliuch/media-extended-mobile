...
- Android 可选安装独立的 Media Extended 截图助手 APK。“辅助 APK 一键截图”按钮使用系统 MediaProjection 授权抓取当前屏幕，自动裁剪播放器、识别当前视频时间并插入此前聚焦的笔记。1.0.3 起可选启用无障碍服务：首张截图没有可读时间时，助手会自动点击视频使控制条显示，再用第二张截图识别时间。1.0.4 起新增独立的“高清源视频帧”按钮；1.0.5 起由内置 yt-dlp 解析 H.264/MP4 视频源，再由 FFmpeg 快速定位并输出指定时间的原尺寸 PNG。源帧失败时默认只提示错误，可在插件设置中选择自动回退到屏幕截图。原相机按钮和图片导入按钮均保留。
...
- 辅助 APK 支持 Android 8.0 及以上（全自动显示进度条请使用 1.0.3 或更新版本）。首次使用或录屏会话结束后需要确认 Android 系统录屏授权；自动显示进度条还需要在系统设置中启用“Media Extended 自动显示视频进度”无障碍服务。该服务仅限 Obsidian 处于前台且收到本机会话令牌保护的截图请求时执行一次点击。DRM/`FLAG_SECURE` 视频仍可能得到黑色画面。
- “高清源视频帧”首次使用需要解压内置运行时，解析可能较慢；1.0.5 会至多每天检查一次 yt-dlp 稳定版更新。视频源解析限时 50 秒，FFmpeg 取帧限时 30 秒，任一阶段超时都会终止对应进程，不会阻塞下次截图。YouTube 的部分格式可能受 PO Token 限制，B站登录/大会员清晰度不会自动继承 Obsidian WebView 的 Cookie。通用辅助 APK 约 215 MB，默认优先兼容 Android 8.1 的 H.264/MP4 视频流。
