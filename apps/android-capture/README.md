...
5. 1.0.4 起可点击单独的“高清源视频帧”：助手用内置 yt-dlp 解析 YouTube/B站 H.264/MP4 视频源，并用 Android 原生解码器按当前时间输出无损 PNG。源帧失败或 75 秒超时默认只提示错误；可在插件设置中启用自动回退到屏幕截图。
...
- 高清源帧不继承 Obsidian WebView 的登录 Cookie，因此 YouTube PO Token、B站登录或大会员限定的清晰度可能不可用。首次运行还需要解压内置 Python/yt-dlp/QuickJS 运行时，APK 和初始化耗时会明显增加。

## 许可证说明

截图助手 1.0.4 起链接 GPL-3.0 的 `youtubedl-android` 0.18.1，因此发布的辅助 APK 作为组合程序按 GPL-3.0 分发；Obsidian 插件其余代码仍保持仓库根目录所述的 MIT 许可证。依赖和对应源码见 `app/src/main/assets/THIRD_PARTY_NOTICES.txt`。
